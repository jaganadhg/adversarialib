...
def apply_only_increment(root_pattern, pattern, grad_update, grad_step, bound_no, only_increment_step, weights, inv_weights, feature_upper_bound=None):
    grad_update_with_indexes = list(enumerate(grad_update*inv_weights)) #ATTENZIONE. IL GRADIENTE VA MOLT PER I PESI/STEP SULLE SINGOLE FEATURE
    ...
    for feature_index, value in grad_update_with_indexes:
        if value > 0: # it means that we should DECREMENT
            ...
            if (root_pattern[feature_index] <= new_value): # we are under "only increment" and normalization is OK...
            ...
            if (sum(diff_pattern) < only_increment_step*bound_no) and (feature_upper_bound is None or (new_value <= feature_upper_bound)): # we are under the boundary and upper_bound is OK...
...
def apply_hamming(root_pattern, pattern, grad_update, grad_step, bound_no):
    ...
    for feature_index, value in grad_update_with_indexes:
        if value > 0: # it means that we should DECREMENT
            ...
            if sum(absolute(pattern-root_pattern)) < bound_no and new_pattern[feature_index] == 0: # we are under the boundary
