...
def get_fname_dataset(folder, dset_name):
    return check_fname(os.path.join(folder, ".".join([dset_name, "txt"])))

def get_fname_exp(exp_path, dset_name, **kwargs):
    ...
    if indexes == 'kfold': # kfold indexes
        ...
        if feed_type == TESTS:
            split_type = 'ts'
        assert(split_type in ('ts', 'tr'))
        fname_args = [class_type, dset_name, str(split_no), split_type]
        if exp_type == ATTACK:
    ...
    fpath_args = [exp_path, feed_type, exp_type]
...
def get_classifiers(exp_path, exp_type, attack_type=None):
    classifiers = dict()
    folder_args = [exp_path, CLASSIFIERS, exp_type]
    if exp_type == ATTACK:
        assert(attack_type is not None)
